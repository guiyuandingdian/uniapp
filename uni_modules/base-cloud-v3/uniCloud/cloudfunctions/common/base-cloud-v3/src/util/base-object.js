const { isObject , isNull , isArray , isFn , isReg , isDate } = require("./base-type") ;
...
function keep(data,keepKeys) {
	if (!keepKeys) {
		return data ;
	}
	let keys = keepKeys.split(",");
	for (const key in data) {
		if (!keys.includes(key)) {
			delete data[key] ;
		}else if (key.indexOf(".") > -1) {
			setDeepValue(data , key , data[key]) ;
			delete data[key] ;
		}
	}
	return data ;
