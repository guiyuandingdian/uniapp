const {parseDate} = require("./base-parse-date.js") ;
const { isObject } = require("./base-type.js") ;
...
function differDate({date , differ , format }) {
	...
	for (var i = differIndex ; i < differs.length; i++) {
		...
		if(time > min){
			number = time ;
			quantifier = timeNames[curDiffer] ;
			break ;
		}
		if(curDiffer == 'second'){
			number = '' ;
...
function isSame (fmt , compareDate , sourceDate) {
	return format(compareDate , fmt ) == format(sourceDate , fmt );
}

function isSameDay (compareDate , sourceDate) {
	return isSame("day" , compareDate , sourceDate);
}

function isSameMonth (compareDate , sourceDate) {
	return isSame("month" , compareDate , sourceDate);
}

function isSameYear (compareDate , sourceDate) {
	return isSame("Year" , compareDate , sourceDate);
}
