/**
 * 该SDK由来自插件市场的插件改造：https://ext.dcloud.net.cn/plugin?id=1645
 */
// #ifdef APP-PLUS
let alphaBg, shareMenu, showState = false , shareConfig = {} ;
// 关闭弹窗
function closeShare(){
...
let otherShareList = [
	{
		icon: "/uni_modules/baker-app/components/baker-share/static/album.png",
		text: "保存到相册",
		provider: "saveToAlbum",
		onClick: shareConfig.saveToAlbum
	},
	
	// {
	// 	icon: "/uni_modules/baker-app/components/baker-share/static/icon_copy.png",
	// 	text: "复制",
	// 	provider: "copy",
	// 	onClick: onCopy
	// },
	// {
	// 	icon: "/uni_modules/baker-app/components/baker-share/static/icon_more.png",
	// 	text: "更多",
	// 	provider: "more",
	// 	onClick: onMore
	// }
...
uni.getProvider({
	...
	success: function (res) {
		if (res.provider.includes('sinaweibo')) {
			platformShareList = [{
				icon: "/uni_modules/baker-app/components/baker-share/static/icon_weibo.png", 
				text: "新浪微博",
				onClick: onShare,
				provider: "sinaweibo",
				type: 0
		...
		if (res.provider.includes('qq')) {
			platformShareList = [{
				icon: "/uni_modules/baker-app/components/baker-share/static/icon_qq.png",
				text: "QQ",
				onClick: onShare,
				provider: "qq",
				type: 1
		...
		if (res.provider.includes('weixin')) {
			platformShareList = [{
				icon: "/uni_modules/baker-app/components/baker-share/static/icon_weixin.png",
				text: "微信好友",
				onClick: onShare,
				provider: "weixin",
			...
			{
				icon: "/uni_modules/baker-app/components/baker-share/static/icon_pengyouquan.png",
				text: "朋友圈",
				onClick: onShare,
				provider: "weixin",
			...
			{
				icon: "/uni_modules/baker-app/components/baker-share/static/ic_xiaochengxu.png",
				text: "小程序",
				onClick: onShare,
				provider: "weixin",
				scene: "WXSceneSession",
...
export default function (shareInfo, callback , config = {} ) {
	shareConfig = {...config};
	...
	alphaBg && alphaBg.show();
	shareMenu && shareMenu.show();
	showState = true;
	return {
		close: function(){
			closeShare();
