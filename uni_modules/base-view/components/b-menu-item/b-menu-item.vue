	...
	<view class="b-menu-item">
		...
		<view @tap="select" class="flex" :class="_titleClass" :style="_titleStyle">
			...
			<block v-if="isCollapse">
				<view class="lh1" v-if="!!icon" :class="icon"></view>
			</block>
			<block v-else>
				<view class="flex lt">
					<view v-if="!!icon" class="mr10 lh1" :class="icon"></view>
					<view v-if="!isCollapse">
						{{title}}
					</view>
				</view>
				<view class="fz12" v-if="!!arrow && !isCollapse">
		...
		<block v-if="slots.default">
			<view :class="{'none' : isCollapse || !isOpened }">
				<slot></slot>
			</view>
		</block>
...
<script>
	import subMenuMixins from "../b-sub-menu/b-sub-menu-mixins.js" ;
	/**
	...
	export default {
		...
		props:{
			bgColor : {
				type : String ,
				default(){
					return uni.$b.getValue("components.menuItem.bgColor")
				}
			},
			color : {
				type : String ,
				default(){
					return uni.$b.getValue("components.menuItem.color")
				}
			},
			activeBgColor : {
				type : String ,
				default(){
					return uni.$b.getValue("components.menuItem.activeBgColor")
				}
			},
			activeColor : {
				type : String ,
				default(){
					return uni.$b.getValue("components.menuItem.activeColor")
				}
			},
			titleClass : {
				type : String ,
				default(){
					return uni.$b.getValue("components.menuItem.titleClass")
				}
			},
			activeClass : {
				type : String ,
				default(){
					return uni.$b.getValue("components.menuItem.activeClass")
				}
			},
			height:{
				type : [String,Number,Array] ,
				default(){
					return uni.$b.getValue("components.menuItem.height")
				}
			}
		},
		data() {
		...
		computed:{
			isOpened(){
				return this.storeData.openIds.includes(this.mid);
			},
			isCollapse(){
				return this.storeData.collapsed ;
			}
		},
		methods: {
			$onSelect(){
				if ( !this.url && this.slots.default ) {
					this.parent.toggle(this.mid);
				}
			}
		}
	}
