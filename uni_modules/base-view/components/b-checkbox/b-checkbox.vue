<template>
	<view class="b-checkbox form-item" :class="outerClassName" v-if="hasAuth">
		<b-label
			:title="title" 
...
<script>
	import radio from "../b-radio/b-radio.js" ;
	/**
	* @description Checkbox 复选框，复选框选项支持独占一行、一行多个选项的布局方式；在选项列表数据渲染方面，支持从本地、远程接口两种方式渲染列表数据；它允许限制最小、最大选择选项数量；在权限控制方面，你可以仅将部分选项展示给拥有某项权限的用户。
	* @tutorial    https://static-bb1f6ced-c751-4549-bcfc-10cd6eb9240e.bspapp.com/#/pages/doc/checkbox
	* @version     1.0.0
	* 
	* @ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Slot 插槽<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	* 
	...
	* @slot     {Scoped}               option              用于自定义选项内容的插槽。
	* 
	* @ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Events 事件<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	* 
	...
	* @method                          select()            选中指定选项时使用，通常用于初始化默认选中值使用。 ( index )
	* 
	* @ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Props 基础属性<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	* 
	...
	* @property {Array}                list                复选框数据列表，mode为local时传入有效。若mode为remote时，服务端返回的数据列表的结构应与list一致。
	...
	* @property {String|Array}         v-model             双向绑定复选框的值，提交表单的场景推荐使用form组件的setData()方法回填表单数据。
	...
	* @property {String|Number|Array}  radius              【响应式属性】 复选框的圆角大小，默认4px。
	* @property {String|Number}        scale               尺寸缩放比例，默认1。
	* @property {String}               type                样式类型，可使用option插槽自定义更丰富的样式。
	*    @value                        normal              默认样式
	*    @value                        block               选择框位于右侧，通常与width="100%"搭配使用。
	*    @value                        badge               右下角角标样式
	*    @value                        tag                 标签样式
	* 
	* @ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Props 标题属性<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	*/
	export default {
		...
		computed:{
			...
			_errorMsg(){
				let msg = {
					minLength : uni.$b.localeB("validate.minChoose", {title : this.title , minlength : this._minlength }) 
				};
