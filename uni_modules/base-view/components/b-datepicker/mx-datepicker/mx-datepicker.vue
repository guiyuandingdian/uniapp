<template>
	<view v-if="isShow" class="picker" :class="{'shadow' : showShadow}">
		<!-- 日期选择器 -->
		<view v-if="type!='time'" class="picker-modal">
			<view class="picker-modal-header">
				<view class="picker-icon picker-icon-zuozuo" 
					:hover-stay-time="100" hover-class="picker-icon-active" 
					@touchstart.stop.prevent="_startSetYear(-1)"
					@mousedown.stop.prevent="_startSetYear(-1)"
					@mouseup.stop.prevent="_stopSet"
					@touchend.stop.prevent="_stopSet"></view>
				<view class="picker-icon picker-icon-zuo" :hover-stay-time="100" hover-class="picker-icon-active" 
					@touchstart.stop.prevent="_startSetMonth(-1)"
					@mousedown.stop.prevent="_startSetMonth(-1)"
					@mouseup.stop.prevent="_stopSet"
					@touchend.stop.prevent="_stopSet"></view>
			...
			<swiper class="picker-modal-body" :circular="true" :duration="200" :skip-hidden-item-layout="true" :current="calendarIndex" @change="onSwiperChange">
				<swiper-item class="picker-calendar" v-for="(calendar,calendarIndex2) in calendars" :key="calendarIndex2">
					...
					<view class="picker-calendar-view" v-for="(date,dateIndex) in calendar" :key="dateIndex" @click="onSelectDate(date)">
						...
						<!-- 正常和选中样式 -->
						<view class="picker-calendar-view-item" :style="{opacity: date.statusStyle.opacity, color: date.statusStyle.color, background: date.statusStyle.background , cursor : date.statusStyle.cursor }">
			...
			<view class="picker-modal-footer">
				<view class="picker-modal-footer-info">
					<block v-if="isMultiSelect">
						<view class="picker-display">
							...
							<text class="picker-display-text hover8">{{BeginTitle}}</text>
							<view v-if="isContainTime" class="picker-display-link hover8" :hover-stay-time="100" hover-class="picker-display-link-active"
							 :style="{color}" @click="onShowTimePicker('begin')">{{BeginTimeTitle}}</view>
						</view>
						<view class="picker-display">
							<text>{{endText}}日期</text>
							<text class="picker-display-text hover8">{{EndTitle}}</text>
							<view v-if="isContainTime" class="picker-display-link hover8" :hover-stay-time="100" hover-class="picker-display-link-active"
				...
				<view class="picker-modal-footer-btn">
					<view class="picker-btn hover8" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancel">{{cancelText}}</view>
					<view class="picker-btn hover8" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirm">确定</view>
				</view>
			</view>
		</view>
		<!-- 时间选择器 -->
		<view v-if="showTimePicker" class="picker-modal picker-time">
			<view class="picker-modal-header">
			...
			<view class="picker-modal-footer">
				...
				<view class="picker-modal-footer-btn">
					<view class="picker-btn hover8" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancelTime">{{cancelText}}</view>
					<view class="picker-btn hover8" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirmTime">确定</view>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			setValue(value) {
				...
				let parseDateStr = uni.$b.parseDate;
				if (value) {
					if (this.isMultiSelect) {
						...
						if (this.type == 'time') {
							...
							let date = parseDateStr(`2021/01/01 ${value}`)
			...
			_startSetYear(value){
				this._startSet(value , "onSetYear");
			},
			_startSetMonth(value){
				this._startSet(value , "onSetMonth");
			},
			_startSet(value , fnName){
				this[fnName](value);
				let times = 1 ;
				this.timer = setInterval(()=>{
					if (times % 10 === 0 && times < 50) {
						value += value ;
					}
					this[fnName](value);
					times ++ ;
			...
			procCalendar(item) {
				...
				item.statusStyle = {
					opacity: 1,
					color: item.disabled ? '#ddd' : (item.isOtherMonth ? "#777" : '#000'),
			...
			_getCalendar(date, proc){
				return DateTools.getCalendar(date, proc , this._start , this._end );
			},
			...
			refreshCalendars(refresh = false) {
				...
				if (this.calendarIndex == 0) {
					if(refresh) this.calendars.splice(0, 1, this._getCalendar(date, this.procCalendar));
					this.calendars.splice(1, 1, this._getCalendar(after, this.procCalendar));
					this.calendars.splice(2, 1, this._getCalendar(before, this.procCalendar));
				} else if (this.calendarIndex == 1) {
					this.calendars.splice(0, 1, this._getCalendar(before, this.procCalendar));
					if(refresh) this.calendars.splice(1, 1, this._getCalendar(date, this.procCalendar));
					this.calendars.splice(2, 1, this._getCalendar(after, this.procCalendar));
				} else if (this.calendarIndex == 2) {
					this.calendars.splice(0, 1, this._getCalendar(after, this.procCalendar));
					this.calendars.splice(1, 1, this._getCalendar(before, this.procCalendar));
					if(refresh) this.calendars.splice(2, 1, this._getCalendar(date, this.procCalendar));
				}
			...
			//校验时间
			_checkTime(){
				if(uni.$b.isNull(this.start) && uni.$b.isNull(this.end)){
					return ;
				}
				let date = this.checkeds.length == 0 ? new Date() : this.checkeds[this.timeType == 'begin' ? 0 : 1] ;
				this.fillTime(date , this.timeValue);
				let timestamp = date.getTime() ;
				if(uni.$b.notNull(this.start) && timestamp < this.start){
					this.timeValue = this._startValues ;
					return ;
				}
				if(uni.$b.notNull(this.end) && timestamp > this.end){
					this.timeValue = this._endValues ;
				}
			},
			...
			onConfirm() {
				...
				if (this.type == 'time') {
					let date = new Date();
					this.fillTime(date, this.beginTime);
					...
					if (this.isMultiSelect) {
						...
						this.checkeds.forEach((date, index) => {
							...
							if (this.isContainTime) {
								...
								this.fillTime(newDate, time[index]);
							}
		...
		computed: {
			...
			_endValues(){
				return this._getValues(this.end);
			},
			_startValues(){
				return this._getValues(this.start);
			}
		},
		watch: {
			show:{
...
<style lang="scss" scoped>
	...
	.picker {
		...
		font-size: 28rpx;
		min-width: calc($calendar-size + 40rpx);
		padding: 0 20rpx;
		min-height: 812rpx;
		...
		&-time {
			...
			border-radius: 12rpx;
			width: $calendar-size;
			margin-left: $calendar-size * -0.5 !important;	
		}
